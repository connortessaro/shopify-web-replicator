...
- The API auto-runs a deterministic local pipeline through `analysis`, `mapping`, `theme_generation`, `store_setup`, and `review`.
...
- The job payload now carries a typed store setup plan for products, collections, menus, and structured content.
...
4. Review the generated artifacts, validation output, and store setup plan in the handoff view.
5. Run `shopify theme dev` inside the configured theme workspace and verify layout parity, content wiring, planned products and collections, CTA behavior, and cart-to-checkout handoff.

## Current limits

- The pipeline is deterministic and local-only; it does not live-crawl or screenshot the reference site.
- The current analysis requires one of the supported page types and writes only the stable generated section, template, and store setup plan outputs for that type.
